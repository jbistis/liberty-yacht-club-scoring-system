/**
 * Liberty Yacht Club Racing Scoring System - 2025
...
function calculateRaceResults() {
  ...
  // Group by race and class, then rank (only non-practice races)
  const raceGroups = {};
  results.forEach(r => {
  ...
  results.forEach(r => {
    calcSheet.appendRow([
      ...
      r.points !== null && r.points !== undefined ? String(r.points) : '',  // force string
      r.status
    ]);
  });
...
function calculateSeriesStandings() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // row[5] = Course
    const boatName = row[6];  // Boat Name
    const raceNum  = row[1];
    const points   = Number(row[13]); // Points
    const status   = row[14]; // Status
...
function calculateSeasonStandings() {
  ...
  // Helper: get date string YYYY-MM-DD from a Date or date-like value
  function toDateKey(val) {
    if (!val) return '';
    const d = val instanceof Date ? val : new Date(val);
    if (isNaN(d.getTime())) return '';
    if (d.getFullYear() < 2000) return ''; // guard against Unix epoch zero (1969-12-31) from empty date cells
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const startDateTime = row[0]; // Start DateTime
    const classNum      = row[4]; // Class
  ...
  // After Pass 1, log all race days per class
  ...
  // Pass 2: for each boat, track which Race Days they participated on
  const classGroups = {};
  const boatRaceDays = {};
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // BYE: counts toward participation threshold
    if (status === 'BYE') {
      boatExcusedDays[boatKey].add(dateKey);
    }

    // Only add points for non-practice, non-DNC, non-BYE
    if (!isPractice && status !== 'DNC' && status !== 'BYE') {
  ...
  seasonSheet.clear();
  let currentRow = 1;
  seasonSheet.getRange(1, 10, 1000, 1).setNumberFormat('@'); // Format col J as text before writing
...
function calculateCumulativeResults() {
  ...
  // Create or clear Cumulative Results sheet
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const raceNum  = row[1];   // Race#
    const series   = row[2];   // Series
    const raceType = row[3];   // RaceType
    const classNum = row[4];   // Class
    const boatName = row[6];   // Boat Name
    const points   = Number(row[13]); // Points
  ...
  Object.keys(structure).sort((a, b) => Number(a) - Number(b)).forEach(series => {
    ...
    Object.keys(classes).sort((a, b) => Number(a) - Number(b)).forEach(classNum => {
      ...
      // Re-scan data to get Sail# and PHRF for each boat
      const boatInfo = {};
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        const bn = row[6];  // Boat Name (col G, index 6)
        if (!boatInfo[bn]) {
          boatInfo[bn] = { 
            sailNumber: row[7],  // Sail# (col H, index 7)
            phrf: row[8]         // PHRF (col I, index 8)
          };
        }
      }
  ...
  cumSheet.getRange(1, 3, currentRow, 1).setNumberFormat('0'); // Force PHRF col as number
